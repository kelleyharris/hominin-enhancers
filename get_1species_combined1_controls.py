...
if len(sys.argv)>3:
    infile=open('allpop_component_category_counts_'+popul+'_'+species+'_1minus2_snps.txt')
else:
    infile=open('category_counts_'+popul+'_'+species+'_1minus2_snps.txt')
...
#    for species in ['neandertal','denisova']:
    ...
    while cind<len(control_lines[0]):
        ...
        cind+=1
        if float(s[-4])+float(s2[-4])==0 and int(s[6])<=categories[-1][0] and int(s[6])>=1 and len(bstat_range[int(s[6])])>0:
...
if len(sys.argv)>3:
    outfile=open('allpop_component_'+popul+'_'+species+'_1minus2_controls.txt','w')
else:
    outfile=open(popul+'_'+species+'_1minus2_controls.txt','w')
