...
popul=sys.argv[1]
infile=open('bstat_freq_sorted_'+popul+'_combined_archaic_1minus2.txt')
...
for line in lines:
    s=line.strip('\n').split('\t')
    if not (int(s[6]),int(s[-1]))==categories[-1]:
        categories.append((int(s[6]),int(s[-1])))
...
outfile=open('category_counts_'+popul+'_1minus2_snps.txt','w')
